...
public class ServerSpecificTaskLauncher extends AbstractTaskExecutor {
   ...
   public void execute(SailPointContext context, TaskSchedule taskSchedule,
      ...
      TaskDefinition taskDefinitionObj = context.getObjectById(
            TaskDefinition.class, taskDefinition);

      String taskName = taskDefinitionObj.getName();

      // Servers are passed in as a csv of IDs. Need a list of server names.
      ...
      if (null != servers && !servers.isEmpty()) {
         ...
         for (String serverId : serverIds) {
            String serverName = context.getObjectById(Server.class, serverId)
                  .getName();
